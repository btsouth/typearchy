...
export default function ConnectForm({ initialCode }: { initialCode: string }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try { const response = await fetch('/api/connect/claim', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ code, handle }) }); const data = await response.json() as { error?: string; handle: string; recoveryCode: string; profileUrl: string }; if (!response.ok) throw new Error(data.error || 'Could not connect profile'); setConnected(data); }
