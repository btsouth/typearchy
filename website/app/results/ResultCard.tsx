...
export default function ResultCard({ result, privateResult = false }: { result: ResultCardData; privateResult?: boolean }) {
  ...
  return <article className="live-result-card" style={themeStyle(result.theme)} aria-label={`Result by ${result.handle}`}>
    ...
    <div className="live-result-actions"><a className="result-race-button" href={result.challengeUrl}>{privateResult ? 'Try this challenge' : 'Race this run'} <span>↗</span></a>{!privateResult && <><button type="button" disabled={!clientReady} onClick={()=>void share()}>Share result</button><button type="button" disabled={!clientReady} onClick={()=>void copyLink()}>{copied ? 'Link copied' : 'Copy link'}</button></>}{result.standingsUrl && <a href={result.standingsUrl}>Standings</a>}</div>
    {!!points.length && <section className="live-result-chart" aria-label={result.progress ? 'Passage progress replay' : 'Words per minute over time'}>
      ...
      {result.progress && <div className="live-result-playback"><button type="button" disabled={!clientReady} onClick={togglePlayback}>{!clientReady ? 'Loading replay…' : playing ? 'Pause' : elapsed>=result.durationMs ? 'Replay' : 'Play run'}</button><input disabled={!clientReady} type="range" min="0" max={result.durationMs} step="10" value={elapsed} onChange={event=>seek(Number(event.target.value))} aria-label="Replay position" aria-valuetext={`${(elapsed/1000).toFixed(1)} seconds`} /><select disabled={!clientReady} value={speed} onChange={event=>setSpeed(Number(event.target.value))} aria-label="Playback speed"><option value={1}>1×</option><option value={2}>2×</option><option value={4}>4×</option></select></div>}
    </section>}
    ...
    {message && <p role="status">{message}</p>}{privateResult ? <p>Only you can open this result. Manage sharing in your challenge history.</p> : <a className="live-result-url" href={result.url}>{result.url.replace('https://','')}</a>}
  </article>;
}
