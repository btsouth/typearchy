...
export default function PracticeHistory({ history, onImport, onRetest, onClear, canRetest }: {
  ...
  const trend = shown.filter(run => !run.interrupted && run.completed !== false).slice(0,12).reverse();
  ...
  return <div className="web-game-history" onClick={event=>event.stopPropagation()}>
    ...
    <p>Practice history stays on this device. Export a backup to move it between the app and browser. Select a run to view or share it.</p>
    <input ref={file} type="file" accept="application/json,.json" hidden onChange={async event=>{
      ...
      setError(''); setNotice('');
      try { if(selected.size>2_000_000) throw new Error('Choose a backup under 2 MB'); const runs=parsePracticeBackup(await selected.text()); onImport(runs); setNotice('Backup merged. Duplicate runs were kept once.'); }
    ...
    {filter !== 'all' && trend.length > 0 && <div className="practice-trend"><p>Best: {Math.max(...shown.filter(run => !run.interrupted && run.completed !== false).map(run=>run.wpm))} WPM · Recent {trend.length} runs</p>
    ...
    {shown.length ? <div className="web-game-history-list">{shown.map(run=><button type="button" key={run.id} title={canRetest(run) ? "View result" : "View result. The original passage is unavailable for retry."} onClick={()=>onRetest(run)}><span>{new Date(run.timestamp).toLocaleDateString(undefined,{month:'short',day:'numeric'}).toUpperCase()}</span><strong>{run.mode.toUpperCase()} / {run.target}</strong><b>{run.wpm} WPM<small>{run.interrupted ? "Paused" : run.publicSlug ? "Shared" : "Local"}</small></b><i>{run.accuracy}%</i></button>)}</div> : <div className="web-game-empty">FINISH A TEST TO START LOCAL HISTORY.</div>}
  </div>;
}
