...
try {
 ...
 await page.locator('input[type=file]').setInputFiles({name:'backup.json',mimeType:'application/json',buffer:Buffer.from(body)});await expect(page.getByRole('status').filter({hasText:'Existing runs were preserved'})).toContainText('Existing runs were preserved');await expect(page.locator('.history-overview')).toContainText('607');await context.close();
